...
// B4 (lộ ra ở lần chạy thật đầu tiên) — DÒNG ĐẦU CỦA EVIDENCE LOG PHẢI LÀ LỆNH
// TÁI LẬP ĐƯỢC LẦN CHẠY NÀY.
// Bản cũ in `pp gate <stage> --tier <tier>`: thiếu tên feature (nên gõ lại là
// exit 2 kèm usage), và dùng đúng cái flag mà `pp gate` đang bỏ qua im lặng —
// với `tier=t2` nó còn in ra một lệnh mà `pp gate` KHÔNG BAO GIỜ chạy được. Một
// header evidence sai lệnh còn tệ hơn không có header: nó mời người đọc gõ một
// thứ không tái lập được lần chạy đã ghi bên dưới.
//
// Tên feature không được truyền vào đây, nhưng nó là chính tên thư mục —
// `featureDir` luôn được dựng bằng `join(root, 'features', feature)`.
//
// Tier nào KHÔNG có dòng lệnh trong bảng này thì nói thẳng là không có, thay vì
// đoán một lệnh gần đúng: đó đúng là cách bản cũ tạo ra header sai.
const REPRO = {
  t1: (f, s) => `pp gate ${f} ${s}`,
  t2: (f, s) => `pp review-record ${f} ${s} --verdict <file.json>`,
}

export function reproCommand(feature, stageId, tier) {
  const fn = REPRO[tier]
  return fn
    ? fn(feature, stageId)
    : `(tier "${tier}" chưa có lệnh tái lập — thêm vào REPRO trong lib/evidence.js)`
}

export function newEvidence(featureDir, stageId, tier, attempt) {
  // N10: mkdir dời xuống `finish` — tạo ev giờ là thuần bộ nhớ. `gate --dry-run`
  // dựng ev để chạy check Y NHƯT gate thật rồi vứt kết quả, nên nó phải không
  // để lại dấu vết nào trên đĩa (kể cả một `.evidence/` rỗng).
  const lines = [`[${new Date().toISOString()}]  ${reproCommand(basename(featureDir), stageId, tier)}`]
  const ev = {
    failed: false,
    // N10: dry-run tóm tắt "bao nhiêu check, bao nhiêu đỏ" mà không in lại
    // cả log — đếm ngay lúc record, cùng một chỗ set `failed`.
    records: 0,
    failures: 0,
