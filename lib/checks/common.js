...
export function checkSectionChecklist(text, file, { name, heading, label, items = [] }) {
  ...
  if (section === null) {
    // Một dòng duy nhất, không lặp N lần cùng một lý do: khi heading chưa có
    // thì MỌI mục đều chưa kiểm được — liệt kê từng mục chỉ tạo nhiễu (lab
    // 2026-08-21: thiếu "## Rủi ro" in 8 dòng chỉ khác nhau mỗi tên mục).
    if (items.length > 0) {
      messages.push(
        `${file}: không tìm thấy heading "${heading}" — ${items.length} ${label} chưa kiểm được, ` +
          `mỗi mục cần một dòng "<mục>: <kết luận>" trong section này`,
      )
    }
