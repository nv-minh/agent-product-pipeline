...
export function checkEars(text, file) {
  ...
  for (const ac of acs) {
    ...
    // Cùng chuẩn khoảng trắng với EARS phía dưới (`\s+`): AC wrap dòng GIỮA
    // "THE" và "SYSTEM" (hoặc "SYSTEM" và "SHALL") vẫn là một SHALL duy nhất —
    // đếm theo literal khoảng trắng đơn sẽ báo "thiếu" oan (lab 2026-08-21).
    const shalls = (ac.body.match(/THE\s+SYSTEM\s+SHALL/g) ?? []).length
    if (shalls === 0) {
