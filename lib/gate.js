...
import { readState, writeState, hashInputs, hashArtifact, withStateLock } from './state.js'
import { stageOrder } from './config.js'
import { MAX_ATTEMPTS, nextStage } from './plan.js'
...
// Hash artifact ĐANG nằm trên đĩa. Ba giá trị trả về mang ba nghĩa khác hẳn
// nhau, đừng gộp: `<hex>` = artifact có thật kèm dấu vân tay của nó;
// `'missing'` = stage CÓ khai báo artifact nhưng file không tồn tại;
// `null` = stage không khai báo output nào.
...
export function tierPassed(featureDir, config, state, stageId, tier) {
  ...
  if (rec?.result !== 'pass') return false
  if (hasFailure(featureDir, stageId, tier)) return false
  const current = currentArtifactHash(featureDir, config, stageId)
  // F3 — "KHÔNG CÓ ARTIFACT" LÀ MỘT GIÁ TRỊ, VÀ NÓ BẰNG CHÍNH NÓ.
  // Luật của R1 là "không có hash hợp lệ ⇒ tier chưa qua", nhưng `'missing'`
  // lọt qua vì `'missing' === 'missing'`. Quan sát được: một `gate: ["t2"]`
  // viết tay tới `done` chỉ bằng phán quyết của LLM trong khi artifact KHÔNG
  // TỒN TẠI trên đĩa. Artifact không có mặt thì chẳng có gì để tier chứng
  // nhận — tier CHƯA qua, bất kể state ghi hash nào.
  if (current === 'missing') return false
  // `current === null` (`outputs: []`) thì KHÔNG áp luật trên, và đó là chủ ý
  // — đừng nhầm là sót. Một stage không khai báo artifact nào là HÌNH DẠNG
  // CONFIG, không phải đường tấn công: `pipeline.json` được PreToolUse guard
  // chặn ghi, và luật evidence bên dưới vẫn áp nguyên vẹn cho stage đó.
  //
  ...
  // vắng mặt.
  return (rec.artifact_hash ?? null) === current
}
...
export function stageDone(featureDir, config, state, stageId) {
  ...
  for (const tier of requiredTiers(config, stageId)) {
    ...
    if (rec?.result === 'pass' && !hasFailure(featureDir, stageId, tier)) {
      const current = currentArtifactHash(featureDir, config, stageId)
      notes.push(
        // F3: gọi đúng tên sự việc — artifact VẮNG MẶT, không phải "đã bị sửa".
        current === 'missing'
          ? `${tier}: artifact ${artifact} KHÔNG CÓ trên đĩa — không có gì để ${tier} chứng nhận, phải viết artifact rồi chạy lại ${tier}`
          : rec.artifact_hash
...
// ─────────────────────────────────────────────────────────────────────────
// B5 — `pp gate` KHÔNG ÉP THỨ TỰ STAGE, NÊN GÕ THẲNG LỆNH LÀ ĐI VÒNG QUA
// HUMAN GATE #1.
//
// Chuỗi quan sát được trên một feature vừa `pp init` (không sửa tay gì):
//
//   pp gate demo 40-testplan          → exit 0, T1 xanh
//   pp review-record demo 40-testplan → "✓ 40-testplan: done"
//   → STATE: 40-testplan = done, trong khi 10-prd = in_progress, human = (chưa)
//
// Test plan đã `done` trước cả khi PRD nó truy vết tới được người duyệt. Thứ tự
// stage TỪNG chỉ tồn tại trong `nextStage` (`pp status`/`pp advance`) — tức là
// trong phần GỢI Ý, không phải trong phần THI HÀNH; ai gõ lệnh trực tiếp (hoặc
// một agent đọc sai chỉ thị) là đi vòng qua nó. §5 xếp T3 (người) là tầng CÓ
// quyền chặn; một tầng chặn mà bỏ qua được bằng cách gõ lệnh kế tiếp thì không
// phải tầng chặn.
//
// Hàm này trả về stage thượng nguồn ĐẦU TIÊN chưa xong, hoặc null nếu đường đã
// thông. Nó hỏi `stageDone` (đọc lại evidence trên đĩa), KHÔNG đọc trường
// `status` — cùng lý do R4: `done` là kết luận, không phải cờ. Stage `overridden`
// được `stageDone` miễn trừ, nên cửa thoát hiểm có ghi sổ vẫn thông đường.
//
// PHẠM VI, nói rõ để không ai tưởng nhiều hơn: hàm này KHÔNG kiểm stage thượng
// nguồn có `stale` hay không. Trường hợp trực tiếp đã được `inputs_hash` của
// CHÍNH stage hạ nguồn bắt (40-testplan khai `10-prd.md` là input, PRD đổi là
// nó stale ngay). Phần còn lại — 10-prd stale vì `00-brief.md` đổi mà
// `10-prd.md` chưa đổi — là luật lan truyền stale xuống hạ nguồn (spec §7.6),
// một việc riêng, không gộp vào đây.
export function upstreamGap(featureDir, config, state, stageId) {
  if (!config.stages?.[stageId]) return null
  for (const id of stageOrder(config)) {
    ...
    if (!verdict.done) {
      ...
      // Hai chuyện khác nhau phải nói khác nhau (cùng phân biệt như `pp report`):
      // "chưa chạy tới" vs "STATE ghi done mà evidence không còn đỡ được".
      return {
        stage: id,
        why: status === 'done'
          ? `ghi done nhưng evidence hiện tại KHÔNG chứng minh (còn thiếu tier: ${verdict.outstanding.join(', ')})`
          : `chưa done (đang "${status}")`,
      }
    }
    // Human gate: `done` chưa đủ, phải có chữ ký. Đây chính là tầng bị đi vòng.
    if (stage.human && st?.human !== 'approved') {
      return { stage: id, why: 'đã done nhưng CHƯA được người duyệt' }
    }
  }
  return null
}
...
// nhau nữa.
export function statusLine(feature, stageId, config, verdict, opts = {}) {
  if (verdict.done) {
    return `\n✓ ${stageId}: done — mọi tier bắt buộc (${requiredTiers(config, stageId).join(', ')}) đã xanh\n`
  }
  const notes = (verdict.notes ?? []).map((n) => `  ⚠ ${n}\n`).join('')
  // N8 (lab 2026-08-21): dòng lệnh nhánh t1 từng đọc "chạy: pp gate ..." — vô
  // nghĩa đúng lúc người đọc cần nó nhất: họ VỪA chạy `pp gate` và nó đỏ. Lỗi
  // nằm ở artifact (đã in ngay phía trên trong notes/evidence), chạy lại không
  // đổi gì. Caller truyền `t1JustFailed` khi T1 vừa đỏ ở chính lệnh này; các
  // caller khác (review-record) nói "chạy" vẫn đúng — T1 chưa từng chạy.
  const verb = opts.t1JustFailed ? 'sửa artifact theo các lỗi trên rồi chạy lại' : 'chạy'
  // T2 không được chạy trước T1: khi cả hai còn thiếu, lối ra là `pp gate`.
  const how = verdict.outstanding.includes('t1')
    ? `  ${verb}: pp gate ${feature} ${stageId}\n`
...
// C1: cả cụm đọc-tươi → tính → ghi nằm TRONG khoá — đọc tươi thu hẹp cửa sổ
// đua, khoá mới là thứ đóng nó.
export function recordTierRun(featureDir, config, stageId, tier, run) {
  return withStateLock(featureDir, () => recordTierRunLocked(featureDir, config, stageId, tier, run))
}

function recordTierRunLocked(featureDir, config, stageId, tier, { ok, evidence, inputsHash }) {
  ...
  // D4 — `state.current` TỪNG KHÔNG BAO GIỜ ĐƯỢC GHI: mọi STATE.md hiển thị
  // "current: (hoàn tất)" từ lúc init tới hết đời feature. Ai mở file để biết
  // pipeline đang đứng ở đâu bị nó nói dối ngay dòng đầu. `current` = stage kế
  // tiếp theo đúng hàm điều phối (`nextStage`) — null khi mọi stage xong, và
  // writeState in "(hoàn tất)" đúng lúc đó. Lưu ý phạm vi: chỉ `current` được
  // GHI; staleness vẫn là thứ SUY RA mỗi lần đọc (triết lý R4 — đừng lưu một
  // kết luận có thể mục), nên §9.1 "status: stale" cố ý không tồn tại trên đĩa.
  try {
    next.current = nextStage(featureDir, config, next).stage
  } catch {
    // nextStage đọc isStale → hashInputs, hàm này ném khi input bắt buộc biến
    // mất. Không tính được "kế tiếp" thì chỉ ít nhất nói được "đang ở đây".
...
// `_state`: giữ chỗ trong chữ ký (mọi caller đang truyền snapshot) nhưng CỐ Ý
// không dùng — FINDING 1 bên dưới đọc state TƯƠI từ đĩa, không tin bản chụp.
export function runT1(featureDir, config, _state, stageId, checks, opts = {}) {
  ...
  // N10: dry-run chạy ĐÚNG tập check của gate thật nhưng không ghi gì — không
  // evidence, không attempts, không state, không audit. Lợi ích quan sát được
  // ở lab: người lái muốn biết "artifact của tôi có qua gate không" TRƯỚC khi
  // đốt một lượt attempt thật (§9.1 đếm mọi lần chạy; pipe `| head` cũng từng
  // bị đếm là một lần đỏ). attempts của dry-run tính như lần CHẠY THẬT kế tiếp
  // (`+ 1`) để dòng `attempt N/M` trong report đúng với lần chạy sắp tới.
  ...
  const ok = !ev.failed
  // N10: dry-run trả báo cáo dạng chữ + bộ đếm rồi dừng — `finish` (ghi file)
  // và `recordTierRun` (ghi state) là CHÍNH RANH GIỚI giữa "thử" và "chạy".
  if (opts.dryRun) {
    return {
      ok, dryRun: true, attempt,
      report: ev.report(), records: ev.records, failures: ev.failures,
    }
  }
