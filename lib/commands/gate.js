...
import { runT1, requiredTiers } from '../gate.js'
import { checksFor } from '../registry.js'
import { outstandingNote } from './review.js'
...
export function gateCmd(args, { root }) {
  ...
  // FIX review cuối (finding 4e): T1 xanh KHÔNG có nghĩa là stage xong. Với
  // stage khai báo `gate: ["t1","t2"]`, `pp gate` phải nói thẳng ra là stage
  // chưa done và còn thiếu tier nào — nếu không, người/agent đọc "RESULT:
  // PASS" rồi tự kết luận (đúng cái mà thiết kế cấm).
  if (r.ok && !r.done) process.stdout.write(outstandingNote(feature, stageId, r.outstanding))
  else if (r.ok) process.stdout.write(`\n✓ ${stageId}: done — mọi tier bắt buộc (${requiredTiers(config, stageId).join(', ')}) đã xanh\n`)
  return r.ok ? 0 : 1
}
