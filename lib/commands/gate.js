...
export function gateCmd(args, { root, workspace }) {
  ...
  // N10 (lab 2026-08-21): `gate --dry-run` — xem artifact có qua gate không
  // mà KHÔNG đốt một lượt attempt thật (§9.1 đếm mọi lần chạy, kể cả lần quên
  // pipe `| head`). Không evidence, không attempts, không state, không audit —
  // dry-run không phải sự kiện pipeline; ghi audit sẽ làm report "thời gian"
  // và cửa sổ usage-sync trôi theo mỗi lần thử. Ép về boolean bằng cách coi
  // MỌI giá trị (`--dry-run`, `--dry-run=true`, kể cả `--dry-run x`) là bật:
  // nhầm hướng "chạy thật" tốn một lượt attempt, nhầm hướng "dry-run" thì không.
  if (flags['dry-run'] !== undefined) {
    const d = runT1(dir, config, state, stageId, checks, { dryRun: true })
    process.stdout.write(`${d.report}\n`)
    const kq = d.ok ? 'XANH' : `ĐỎ — ${d.failures}/${d.records} check fail`
    process.stdout.write(
      `\n(dry-run) ${stageId}: ${kq} — attempt ${d.attempt}/${MAX_ATTEMPTS} sẽ được dùng khi chạy thật\n` +
      'dry-run không ghi sổ: attempts/evidence/STATE/audit không đổi.\n' +
      `Chạy thật: pp gate ${feature} ${stageId}\n`,
    )
    return d.ok ? 0 : 1
  }
