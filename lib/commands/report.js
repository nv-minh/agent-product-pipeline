...
// N6 (lab 2026-08-21): timestamp audit có milisecond — nhiễu khi đọc bằng mắt,
// không mang thông tin. Bỏ milisecond, giữ nguyên ISO UTC: giờ local sẽ làm
// report phụ thuộc múi giờ máy (test và CI chạy nhiều nơi).
const ngắnISO = (ts) => ts.replace(/\.\d{3}(?=Z|$)/, '')

function printFeature(root, feature, sole) {
  ...
  for (const [id, st] of entries) {
    ...
    if (config?.stages?.[id] && st.status === 'done') {
      ...
      if (!verdict.done) {
        ...
        // không còn chứng minh được chữ done"; dưới là "evidence cũ vẫn hợp lệ,
        // nhưng input thượng nguồn đã đổi sau lần gate xanh cuối cùng".
        status = 'done⚠'
  ...
  if (usage.length > 0) {
    ...
    // N6: gợi ý kèm lệnh cụ thể chỉ khi report đang soi MỘT feature. Trên mặt
    // bằng mọi feature, dòng này lặp N lần y hệt nhau — gộp một dòng cuối.
  ...
  if (audit.length > 0) {
    ...
    if (tss.length > 0) {
      process.stdout.write(`  → thời gian: ${ngắnISO(tss[0])} → ${ngắnISO(tss[tss.length - 1])}\n`)
    }
  }
  for (const { id, verdict, stale } of drift) {
    if (stale) {
      process.stdout.write(
        `  ⚠ stage ${id}: STATE ghi done, evidence cũ vẫn hợp lệ — nhưng INPUT THƯỢNG NGUỒN đã đổi sau lần gate xanh cuối cùng (\`pp status\` báo regate); chạy lại \`pp gate ${feature} ${id}\`\n`,
      )
      continue
    }
    process.stdout.write(
...
export function reportCmd(args, { root }) {
  ...
  const sole = features.length === 1
  let thiếuUsage = 0
  for (const f of features) {
    if (!printFeature(root, f, sole)) thiếuUsage++
  }
  // N6 (lab 2026-08-21): mặt bằng nhiều feature mà chưa feature nào có dữ liệu
  // usage thì gợi ý "chạy pp usage-sync <tên>" lặp N lần y hệt nhau — gộp một
  // dòng cuối; từng feature chỉ in dòng token khi CÓ dữ liệu.
  if (!sole && thiếuUsage > 0) {
    process.stdout.write(
      `\n→ token: ${thiếuUsage}/${features.length} feature chưa có dữ liệu usage — pp usage-sync <feature> để khai thác transcript\n`,
    )
  }
  return 0
}
