...
export function reviewPromptCmd(args, { root }) {
  ...
  //
  // N11 (lab 2026-08-21): việc thu hồi đó từng diễn ra IM LẶNG (ghi đè
  // pending.json). Người gọi phát prompt cho reviewer A, chạy lại review-prompt
  // vì lý do gì đó, và không ai biết prompt trong tay A đã thành giấy vụng —
  // tới lúc verdict của A bị chặn "sai nonce". Nói ra ngay tại điểm thu hồi.
  // Ra STDERR vì stdout của lệnh này là prompt, được copy NGUYÊN VĂN cho
  // reviewer — không được lẫn câu chuyện vận hành vào đó.
  const prevPending = readPending(dir, stageId)
  if (prevPending) {
    process.stderr.write(
      `pp: phiếu review trước của ${stageId} (phát ${prevPending.ts}) đã bị thu hồi bởi lần gọi này — ` +
      'nonce trong prompt cũ không còn hiệu lực. Một prompt một phán quyết: người cầm prompt cũ cần nhận prompt mới.\n',
    )
  }
