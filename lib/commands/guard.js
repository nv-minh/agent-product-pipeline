...
function checkPath(rawPath) {
  ...
  if (FEATURES_AUDIT.test(p)) {
    return block('pp: audit.jsonl chỉ được ghi bởi `pp` — sổ kiểm toán không được sửa tay.\n')
  }
  if (FEATURES_REVIEW_DIR.test(p)) {
    return block('pp: .review/ chỉ được ghi bởi `pp` — bản lưu verdict không được sửa tay.\n')
  }
  if (FEATURES_USAGE_DIR.test(p)) {
    return block('pp: .usage/ chỉ được ghi bởi `pp usage-sync` — token là dữ kiện khai thác được, không phải số tự viết.\n')
  }
  return ALLOW
}
