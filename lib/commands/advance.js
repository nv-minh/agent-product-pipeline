...
import { CLARITY_HEADING } from '../checks/prd.js'
...
function stageSchema(root, stage) {
  ...
  try {
    ...
    return { schema: loadSchema(root, name), warn: null }
  } catch (e) {
    ...
    return { schema: {}, warn: `⚠ schema/${name}.json không đọc được (${e.message}) — chỉ thị dưới đây THIẾU danh sách heading bắt buộc; chạy pp doctor` }
  }
}
...
export function advanceCmd(args, { root, workspace }) {
  ...
  if (!feature) { process.stderr.write('pp advance <feature>\n'); return 2 }
  if (!root) {
  ...
  const { schema, warn: schemaWarn } = stageSchema(root, s)
  if (schemaWarn) process.stdout.write(`${schemaWarn}\n\n`)
  const headings = schema.requiredHeadings ?? []
  // Luật câu hỏi cũng nêu TRƯỚC khi agent viết — không đợi gate đỏ mới dạy.
  // Cùng nguồn schema với gate (minQuestions/clearQuestionsMax), không copy số
  // vào đây. Schema méo (warn ở trên đã nói) thì bỏ dòng thay vì in "undefined".
  const questionsRule =
    s.outputs.includes('10-questions.md') && schema.minQuestions != null
      ? `${schema.minQuestions} câu hỏi phân biệt đã trả lời — hoặc, nếu brief + refs đủ rõ: khối "${CLARITY_HEADING}" (Lý do đủ rõ + Giả định đã xác minh) + tối đa ${schema.clearQuestionsMax} câu verify`
      : null
