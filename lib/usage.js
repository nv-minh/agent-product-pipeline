...
import { createReadStream, readdirSync, readFileSync, existsSync, mkdirSync, appendFileSync } from 'node:fs'
...
// (/Users/abc/x → -Users-abc-x). Chỉ quét đúng dir project này — không đụng
...
function attribute(tsMs, windows, feature, rawLine) {
  for (const w of windows) {
    if (tsMs >= w.start && tsMs < w.end) return { attrib: 'window', stage: w.stage }
  }
  // Fallback: dòng thô nhắc tới feature (tool input, text) → thuộc feature
  // nhưng không rõ stage.
  if (rawLine.includes(`features/${feature}`)) return { attrib: 'mention', stage: null }
  return null // scanned-only: không thuộc feature này
}
...
async function mineFile(filePath, { root, feature, windows, sinceMs, known }) {
  ...
  for await (const line of rl) {
    ...
    if (typeof j.cwd === 'string' && j.cwd !== root) continue
    ...
    const a = attribute(tsMs, windows, feature, line)
    if (!a) continue
